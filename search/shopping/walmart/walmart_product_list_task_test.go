...
func TestShoppingFlow(t *testing.T) {
	...
	engine := WalmartProductListTask{}
	_, res, err := engine.Run(getTestUser(), SHOPPING_ROOM, "I want to buy some crispy cookies")
	if err != nil {
...
func TestShoppingConversationHistory(t *testing.T) {
	...
	saveChatHistory(getTestUser(), SHOPPING_ROOM, query, resp)

	content := conversationContext(getTestUser(), SHOPPING_ROOM)
...
func TestShoppingFlowWithAdvice(t *testing.T) {
	...
	_, res, err := engine.Run(getTestUser(), SHOPPING_ROOM, "存储大一点，像素高的")
	if err != nil {
		t.Error("flow err :", err.Error())
	} else {
		t.Log("result:", res)
